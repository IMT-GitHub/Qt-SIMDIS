...
* EXAMPLE1
*
* Example 1 illustrates the creation of a DCS server capable of receiving 
* clinet connections, processing client requests, removing closed client 
* connections, and sending time updates to clients.  This involves creating 
* and initializing a track database, creating and initializing a DCS server, 
* accepting and negotiating client connections, and checking for client 
* requests and connection closures.  
*
******************************************************************************/
...
int main(int argc, char** argv)
{
  ...
  // Start server here
  //
  success = DCSCom::serverStart(server, server_port, data_type, data_port, data_addr, NULL);
  if (!success)
  {
