...
/**
 * Representation of Status Text that gets used in SIMDIS.  This is a text area
 * that is configurable using a simCore::TextReplacer.  Set up a template status
 * specification, and on each frame the text will update.  For convenience, the
 * class is placed in a MatrixTransform.  Text is aligned lower-left wherever
 * you position the matrix.
 */
class SDKUTIL_EXPORT StatusTextNode : public osg::MatrixTransform {
 public:
   ...
   */
  int setStatusSpec(const std::string& statusSpec,
 ...
 protected:
  ...
  virtual void create_(const std::string& status, const osg::Vec4f& color,
                       const std::string& font, double fontSize);
  /** Update the existing text object  */
  void update_();

  osg::ref_ptr<simUtil::HudColumnText>
      statusHudText_;  ///< Pointer to the HudText that contains the status
                       ///< display
  simCore::TextReplacerPtr
      textReplacer_;        ///< Pointer to the replacer that processes status
                            ///< specification into status text
  std::string statusSpec_;  ///< Current status specifier from which status
                            ///< display is generated
};

/**
 * Class that manages status display info overlay on the specified view.  This
 * is a StatusTextNode that is able to automatically reposition itself inside a
 * View.
 */
class SDKUTIL_EXPORT StatusText : public StatusTextNode {
 public:
  /// Enumeration of positions for status display
  enum Position { LEFT_BOTTOM, LEFT_CENTER, LEFT_TOP };

  /** Constructs a new StatusText */
  StatusText(simVis::View* view, simCore::TextReplacerPtr textReplacer,
             StatusText::Position pos = LEFT_BOTTOM);
 ...
 private:
  osg::observer_ptr<simVis::View>
      view_;  ///< Pointer to the view in which the status will be displayed
  osg::ref_ptr<FrameEventHandler>
      frameEventHandler_;  ///< Pointer to the frame event handler
